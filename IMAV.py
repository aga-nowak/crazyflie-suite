...
class GateIMAV:
    ...
    def log_data(self):
        ...
        self._pylogfile.write(
            f'{sys_time}, {self.time}, {self._data["stateEstimate.x"]}, {self._data["stateEstimate.y"]},'
            f'{self._data["stateEstimate.z"]}, {self._data["jevois.errorx"]}, {self._data["jevois.errory"]},'
            f'{self._data["jevois.width"]}, {self._data["jevois.height"]}, {self.yaw_rate}, {self.z_distance}\n'
        )
    ...
    def fly_through_gate(self, time_limit):
        ...
        while time_passed < time_limit and max(width, height) < 180:
            ...
            # Computing yaw rate and z distance for the commander (filtered)
            alpha_yaw = 0.8
            error_y_gain = 0.5
            alpha_z = 0.8
            error_x_gain = 0.001

            self.yaw_rate = alpha_yaw * self.yaw_rate - (1 - alpha_yaw) * error_y_gain * error_y
            ...
            self._cf.commander.send_zdistance_setpoint(0.0, -10.0, self.yaw_rate, self.z_distance)

            if SHOW_DATA_LIVE:
                if not self.figure_handle:
    ...
    def fly_forward(self, time_limit):
        ...
        while time_passed < time_limit:
            # Commanding roll, pitch, yaw rate and z position
            self._cf.commander.send_zdistance_setpoint(0.0, -10, 0.0, DEFAULT_HEIGHT)
            # log the data:
            self.log_data()
    ...
    def landing(self):
        time_passed = 0.0
        while time_passed < 5 and self._data['stateEstimate.z'] > 0.1:
